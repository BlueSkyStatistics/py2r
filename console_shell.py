...
class RShell(cmd.Cmd):
    ...
    def do_r(self, args):
        ...
        try:
            ...
            print(dumps({
                            "message": f"CLIENT_EXCEPTION (run): unexpected command format {args.encode('utf8')}: exception: {format_exc()}",
                            "type": "exception",
                            "code": 400}))
    ...
    def do_openblankds(self, args):
        ...
        try:
            ...
            print(dumps({
                            "message": f"CLIENT_EXCEPTION (openblank): unexpected command format {args.encode('utf8')}: exception: {format_exc()}",
                            "type": "exception",
                            "code": 500}))
    ...
    def do_getautocompletestrings(self, args):
        ...
        try:
            ...
            print(dumps(
                {"message": f"DATA_EXCEPTION (do_getautocompletestrings): {format_exc()}\n command: {args['cmd']}",
                 "type": "exception",
                 "code": 500}))
            content = ""
    ...
    def do_getpackagesforautocomplete(self, args):
        ...
        try:
            ...
            print(dumps(
                {"message": f"DATA_EXCEPTION (do_getpackagesforautocomplete): {format_exc()}\n command: {args['cmd']}",
                 "type": "exception",
                 "code": 500}))
            content = ""
